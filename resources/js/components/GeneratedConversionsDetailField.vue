<template>
  <panel-item :field="field">
    <div slot="value" class="flex flex-wrap -m-2">
      <div v-for="({previewUrl, copyAsUrl}, name) in conversions" :key="name" v-tooltip="tooltip(name)" class="relative group flex m-2 rounded-full overflow-hidden">
        <img :src="previewUrl" :alt="name" class="w-16 h-16 object-cover">
        <div class="group-hover:block hidden absolute pin bg-overlay">
          <div class="flex items-center justify-center h-full">
            <button type="button" class="flex text-white hover:text-primary focus:outline-none" @click="copy(copyAsUrl)">
